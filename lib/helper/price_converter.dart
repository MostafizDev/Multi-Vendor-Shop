...
class PriceConverter {
  static String convertPrice(
    ...
    return '${Provider.of<SplashProvider>(context, listen: false).currency.symbol}${(price * Provider.of<SplashProvider>(context, listen: false).currency.exchangeRate).toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
  }
  ...
  static String percentageCalculation(BuildContext context, double price,
    ...
    return '$discount${discountType == 'percent' ? '%' : Provider.of<SplashProvider>(
        context,
        listen: false,
      ).currency.symbol} OFF';
  }
